...
def prepare_ml_features(df):
    ...
    try:
        ...
        _pattern_type = df.get('Pattern_Type', pd.Series('Neutral', index=df.index))
        feature_dict['pattern_bullish'] = (_pattern_type == 'Bullish').astype(int)
        feature_dict['pattern_bearish'] = (_pattern_type == 'Bearish').astype(int)
...
def predict_with_ml(df, model=None, regime: str = None):
    """
    Generate ML predictions with confidence score.

    Regime-aware path (retrain_advanced.py outputs):
      1. Load models/training_meta.json to get top_features + regime paths.
      2. If a regime-specific model exists for the current regime, use it.
      3. Otherwise fall back to the global signal_model.ubj.
      4. Apply SHAP top_features filter before DMatrix creation.
    """
    ...
    # ── Try regime-aware model first ─────────────────────────────
    if model is None and regime and os.path.exists(META_PATH):
        try:
            with open(META_PATH) as f:
                meta = _json.load(f)
            regime_key  = regime.upper()
            regime_paths = meta.get('regime_paths', {})
            model_path   = regime_paths.get(regime_key)

            if model_path and os.path.exists(model_path):
                model = xgb.Booster()
                model.load_model(model_path)
                log_message(f"🎯 Regime model [{regime_key}] loaded")
        except Exception as e:
            log_message(f"Regime model load failed ({e}) — using global")
            model = None

    # ── Fall back to global model ─────────────────────────────────
    if model is None:
        global_path = os.path.join(PROJECT_ROOT, 'signal_model.ubj')
        if os.path.exists(global_path):
            model = xgb.Booster()
            model.load_model(global_path)
    ...
    try:
        ...
        # Apply SHAP feature selection if meta exists
        if os.path.exists(META_PATH):
            try:
                ...
                if top_features:
                    available = [c for c in top_features if c in features.columns]
                    if len(available) >= 5:
                        features = features[available]
            except Exception:
                pass  # use full feature set if meta read fails
