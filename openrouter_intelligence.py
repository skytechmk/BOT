...
class OpenRouterIntelligence:
    ...
    def analyze_systemic_fragility(self, context: str = "") -> dict:
        """Assess systemic fragility from AI. Returns severity/fragility_score/risk_level."""
        try:
            prompt = (
                f"You are a macro risk analyst. Based on current crypto market conditions "
                f"({context}), assess systemic fragility. "
                f"Reply in JSON only: {{\"severity\": \"LOW|MODERATE|HIGH|CRITICAL\", "
                f"\"fragility_score\": 0.0-1.0, \"risk_level\": \"low|moderate|high|critical\", "
                f"\"reason\": \"brief reason\"}}"
            )
            text = self.query_ai(prompt, max_tokens=150)
            import json, re
            ...
            pass
        return {"severity": "MODERATE", "fragility_score": 0.5, "risk_level": "moderate", "reason": "fallback"}
